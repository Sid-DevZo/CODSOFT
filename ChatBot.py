...
greeting_inputs = ['hey', 'whats up', 'good morning', 'good evening', 
                   'morning', 'evening', 'hello there', 'hey there']
...
convo_responses = ['Great! what about you?', 'Getting bored at home :( wbu??', 
                   'Not too shabby']

# conversation replies by the user

convo_replies = ['great', 'i am fine', 'fine', 'good', 'super', 
                 'superb', 'super great', 'nice']
...
# fetching raw html data about SciBot from wiki
...
# method to generate response to queries regarding SciBot
...
def generate_response(user):
    ...
    if vector_matched == 0:
        SciBot_response = SciBot_response + 'Sorry, my database doesn\'t have the response for that. Try ' \
                                                'something different and related to SciBot. '
        return SciBot_response
    else:
        SciBot_response = SciBot_response + sentences[similar_sentence_number]
        return SciBot_response
...
continue_chat = True
print('Hi! I am SciBot. You can ask me anything regarding myself and I shall try my best to answer them: ')
while continue_chat:
    ...
    if user_input != 'bye':
        if user_input == 'thanks' or user_input == 'thank you very much' or user_input == 'thank you':
            continue_chat = False
            print('SciBot: Not a problem! (And WELCOME! :D)')
            ...
            if generate_greeting_response(user_input) is not None:
                print('SciBot: ' + generate_greeting_response(user_input))
            elif generate_convo_response(user_input) is not None:
                print('SciBot: ' + generate_convo_response(user_input))
            elif generate_answers(user_input) is not None:
                print('SciBot: ' + generate_answers(user_input))
            else:
                print('SciBot: ', end='')
        ...
        continue_chat = False
        print('SCiBot: Bye, take care, stay home and stay safe!')